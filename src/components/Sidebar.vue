<template>
  <div
    class="sidebar d-flex flex-column flex-shrink-0 px-2 py-3 text-white bg-dark"
  >
    <ul class="sidebar__nav nav nav-pills flex-column mb-auto">
      <li v-for="link in sidebarLinks" :key="link.title" class="nav-item">
        <router-link
          :to="link.url"
          class="nav-link"
          :class="{ active: getActiveStatus(link.url) }"
        >
          <div class="nav-image">
            <img :src="getImageByUrl(link.image)" alt="" />
          </div>
          <div class="nav-title">
            {{ link.title }}
          </div>
    ...
    <div class="">
      <a
        href="https://github.com/Artyrcha"
        target="_blank"
